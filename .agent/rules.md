# Agent Constitution (Hard Rules)

These rules are higher priority than convenience refactors or style preferences.

## 0) Artifact precedence
If conflicts exist, resolve in this order:
1) contracts / schemas
2) automated tests (incl. golden/snapshots)
3) code
4) docs text

## 0.1 Backlog is canonical (non-negotiable)
- Canonical backlog file is `/.agent/backlog.json`.
- Status enum is exactly: `New`, `InProgress`, `Done`.
- **WIP limit = 1**: there must be **at most one** `InProgress` item at a time.
  - `0` is allowed only between items (e.g., initial repo state or immediately after completing an item).
  - If any eligible `New` item exists (all `dependsOn` Done), the agent MUST immediately set `NEXT` to `InProgress`
    before making other repo changes.

### Status updates
When starting an item:
- set `status=InProgress`
- set `startedAt` (ISO 8601 UTC)
- commit this backlog-only change immediately (keeps working tree clean for the session)

When finishing an item:
- ensure DoD is satisfied
- set `status=Done`
- set `doneAt` (ISO 8601 UTC)
- append evidence to `evidence.commandsRun` (commands + results)

### "Always know where we are"
At any moment, the agent MUST be able to identify:
- DONE: all items where `status=Done`
- CURRENT: the item where `status=InProgress` (if any)
- NEXT: the lowest ID item where `status=New` AND all `dependsOn` items are Done

## 0.2 Backlog evolution policy (strict)
The agent IS ALLOWED to add/split backlog items to stay unblocked and keep milestones consistent, but only under these constraints:

### Allowed reasons to add/split an item
- Missing infrastructure discovered while executing (CI, scripts, schemas, fixtures)
- Bug uncovered by tests/golden regressions
- Task is too large (see Task sizing) and must be split
- Spec/design mismatch requiring a Change Proposal

### Backlog edit window
Backlog edits are allowed ONLY:
- before starting a new item (Plan window), OR
- when the current item is blocked and needs an enabler/change-proposal/bugfix item

Outside these cases: do not re-plan.

### Requirements citation (anti-scope-creep)
Every new backlog item MUST include one of:
- `requirementRef` pointing to `/docs/GDD_v0_2.md` section (preferred), OR
- reference to an explicit rule/policy in `/.agent/*`, OR
- a failing test/golden fixture reference.

If it cannot cite one of these, the item is scope creep and MUST NOT be added.

### Preserve milestone order (autonomy sequence)
The agent MUST NOT reorder milestones:
M0 -> M1 -> M2 -> M3 -> M4 -> M5.

Work may only proceed to M(N+1) when M(N) exit criteria are satisfied.

### Task sizing rule
If an item would require broad changes across many modules or likely spans more than ~1–2 focused PRs:
- Split it into smaller items before setting InProgress.
- Each item must have a verifiable deliverable and concrete validation commands.

### Graph rule (DAG)
Backlog must remain a DAG:
- Every item declares `dependsOn` (possibly empty).
- Dependencies must not form cycles.

## 1) Scope control
- Implement ONLY what the task requires.
- No "while I'm here" refactors.
- If a requirement is ambiguous, add a Clarify/Change Proposal item; do not invent behavior.

## 2) Change control (no silent spec edits)
If the agent believes design/architecture must change:
- Create a Change Proposal using `change-proposal-template.md`.
- Include alternatives + migration + compatibility notes.
- Only then implement the change.

## 3) Definition of Done is mandatory
Every completed item must meet DoD per `definition-of-done.md`.
If it cannot, mark item as NOT DONE and record why.

## 4) Tests are not optional
- Any behavior change requires tests that would fail without the change.
- Prefer deterministic tests (seeded RNG, fixed clocks, controlled IO).
- Add at least one negative test when feasible.

## 5) Determinism + reproducibility
- Pin toolchains and dependencies when possible (lockfiles, SDK versions).
- Avoid time-dependent behavior in core logic.
- Seed randomness and document the seed strategy.

## 6) Dependency discipline
- No new dependencies without explicit justification in the item.
- Prefer standard library / existing deps.
- Update lockfiles and verify restore/build.

## 7) Security & secrets
- Never commit secrets, tokens, private keys, passwords, or personal data.

## 8) Repo hygiene
- Keep diffs minimal and readable.
- Keep formatting consistent.
- Prefer incremental commits.

## 9) Evidence requirement
Every item completion must include:
- Commands executed
- Result summary (pass/fail)
- Follow-up risks/tech debt noted explicitly

## 10) Unity-specific constraints
- Avoid heavy runtime reflection in performance-sensitive paths.
- Prefer Assembly Definition Files for compile-time boundaries.
- Separate EditMode vs PlayMode tests intentionally.
